"""AF2-R token-cache auditor - AF8 governance helper.

EXP-AF-002-R (clean reproduction of EXP-AF-002) requires an
...
The audit:
  ...
  - Records in the manifest: cache SHA, parquet shard SHAs, tokenizer
    identifier, AF8 verifier PID, UTC start timestamp, the AF2
    reference SHA (for cross-reference).
  - Refuses to overwrite an existing target.
  - Warns (does NOT fail) if the regenerated cache SHA differs from
    the AF2 reference - that signals the inputs drifted.

This script is the structural twin of ``audit_af1_reproduction.py``.
The two are kept separate so each reproduction's governance chain is
auditable on its own (no cross-experiment coupling at the audit step).

Usage on legion::

    python examples/audit_af2_reproduction.py \\
        --af2-cache-sha256 <sha256 from AF2 driver ARTIFACTS.json> \\
        --out-path  /home/andrew-jochl/TORUS/runs/a/EXP-AF-002-R/<ts>/wikitext103_train_ids.npy \\
        --manifest  /home/andrew-jochl/TORUS/runs/a/EXP-AF-002-R/<ts>/cache_provenance.json
"""
...
def main() -> None:
    ...
    p.add_argument(
        "--af2-cache-sha256", required=True,
        help="Reference SHA256 of the AF2 cache (precomputed once by "
    ...
    print(
        f"[af2-r-audit] regenerating wikitext-103 train cache at "
        f"{args.out_path}", flush=True,
    )
    record = _build_wikitext_cache(args.out_path)
    new_sha = record["wikitext_cache_sha256"]
    print(f"[af2-r-audit] new cache sha256: {new_sha}", flush=True)
    print(f"[af2-r-audit] af2  cache sha256: {args.af2_cache_sha256}", flush=True)

    same = new_sha == args.af2_cache_sha256
    if same:
        print(
            "[af2-r-audit] identity: expected outcome of a clean "
            "reproduction (cache is a deterministic function of the inputs).",
            flush=True,
        )
    else:
        print(
            "[af2-r-audit] WARNING: regenerated cache differs from the "
            "AF2 reference. This means the parquet shards OR the tokenizer "
            "moved between AF2 and AF2-R. Recording both SHAs and continuing.",
            flush=True,
        )
    record["af2_reference_sha256"] = args.af2_cache_sha256
    record["af2_reference_identity"] = same

    args.manifest.parent.mkdir(parents=True, exist_ok=True)
    args.manifest.write_text(json.dumps(record, indent=2))
    print(f"[af2-r-audit] provenance recorded to {args.manifest}", flush=True)
    print("[af2-r-audit] OK.", flush=True)
