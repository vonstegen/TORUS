"""Post-hoc eval of random arms for EXP-RPM-AF2D-CONFIRM-V7.

Mirrors eval_random_v6v2.py but parameterized for the V7 path
(EXP-RPM-AF2D-CONFIRM-V7 instead of EXP-RPM-AF2D-SEVERITY) and
3 thresholds (0.6, 0.8, 1.0).
"""
...
def main():
    ...
    ap.add_argument("--exp-id", default="EXP-RPM-AF2D-CONFIRM-V7")
    ...
    print(f"[eval-random-v7] target={args.target_module} thresholds={thresholds} seeds={seeds}", flush=True)
        ...
        for seed in seeds:
            for arm in arms:
                ...
                if not npz_path.exists():
                    print(f"[eval-random-v7] {arm_dir}: no adapter.npz", flush=True)
                    continue
                ...
                if es_path.exists():
                    ...
                    if existing.get("tasks"):
                        continue
                print(f"[eval-random-v7] thr={thr} seed={seed} arm={arm}", flush=True)
                # Determine model_name from trained arm eval.summary.json
                model_name = None
                for trained_arm in ["t2_ternary", "lora"]:
                    tf = ts / f"seed-{seed:03d}" / trained_arm / "eval.summary.json"
                    if tf.exists():
                        model_name = json.loads(tf.read_text()).get("model")
                        break
                if not model_name:
                    print(f"[eval-random-v7] no model_name found", flush=True)
                    continue
                ...
                print(f"[eval-random-v7] wrote {es_path}", flush=True)
                del model, tokenizer
                gc.collect()
