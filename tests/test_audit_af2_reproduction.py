"""Tests for examples/audit_af2_reproduction.py.

The AF8 governance for AF2-R is *traceability*, not byte-difference:
...
Structural twin of test_audit_af1_reproduction.py.
"""
...
audit = _load("audit_af2_reproduction")
...
def test_main_refuses_to_overwrite_existing(tmp_path: Path) -> None:
    ...
    out = subprocess.run(
        [
            sys.executable, str(EXAMPLES / "audit_af2_reproduction.py"),
            "--af2-cache-sha256", "deadbeef" * 8,
            "--out-path", str(target),
