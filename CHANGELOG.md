...
**Stage 3 v2 EXP-RPM-DAMAGE-MAP-V2 CALIBRATION_GATE_FAIL — Empirical demarcation of damage-mechanism envelope.** Stage A probe at AF2-D/L0/down_proj measured pre-train ppl across MagnitudePrune k ∈ {0.5..0.95} and Dropout p ∈ {0.3..0.99}. **MagnitudePrune max ppl 22.06 (5% of BAND-3 target); Dropout max ppl 69.07 (16% of target). NEITHER new mechanism can produce catastrophic damage at this layer** — only TWN-style per-group absmean-zero and Gaussian signed noise reach BAND-3 magnitude (~ppl 430). Manifest kill criterion #2 triggered: experiment falls back to reporting only {TWN, Gaussian} data (exactly Stage 3 v1). New contribution: T2's Pareto-frontier region is bounded to {TWN, Gaussian}-catastrophic regimes; expanding the mechanism envelope would require either a different layer (where weight statistics differ) or a different damage implementation. Track B gating question is further bounded to P(T2 helps | mechanism ∈ {TWN, Gaussian} × severity ≥ 0.5 × layer × wikitext). **No new tournament data collected** — calibration IS the verdict. Freeze exception: 4 commits added `--damage-magnitude-prune` and `--damage-dropout` to the tournament driver (cac5b1f, a3a338e, b2cc208, b28c258, f092e12). Manifest at `research/residual-pareto/experiments/EXP-RPM-DAMAGE-MAP-V2/manifest.yaml`; calibration table at `calibration_table.md`; verdict at `verdict.md`. **Next: Phase 1 EXP-A-011 (layer sensitivity at TWN).**
